...
func (aad *AAD) UsersGetMembersOf() {
	result, err := aad.graphClient.Users().Get(context.Background(), nil)
	if err != nil {
		fmt.Printf("[UsersGet] err:%v\n", err)
		return
	...
	for i, v := range res {
		...
		fmt.Printf("- v.GetId():%s\n", *(v.GetId()))
		UserID := *(v.GetId())
		result2, err := aad.graphClient.Users().ByUserId(UserID).MemberOf().Get(context.Background(), nil)
		if err != nil {
			fmt.Printf("[UsersGetMembersOf] err:%v\n", err)
			return
		}
		res2 := result2.GetValue()
		for j, v2 := range res2 {
			PrintStr(fmt.Sprintf("UsersGetMembersOf-%d-%d", i, j), v2)
			fmt.Printf("- v2.GetId():%s\n", *(v2.GetId()))
			group, ok := v2.(*models.Group)
			if ok {
